    ...
    <div className="min-h-screen">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="space-y-6">
              <div className="inline-flex items-center gap-2 rounded-full border bg-background/60 backdrop-blur px-4 py-1.5 text-xs font-medium">
                ...
                <span className="text-muted-foreground">Live · 100B events/day · 5 venues · 10 news sources</span>
              </div>
              ...
              <p className="text-lg text-muted-foreground leading-relaxed max-w-xl">
                Multi-page application with live market data, RSS news feeds, file management,
                automation rules, and a synthetic tick lakehouse. Open-source preview, no NDA required.
              </p>
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                ...
                <Button size="lg" variant="ghost" asChild>
                  <Link href="/markets">Live Markets <TrendingUp className="ml-2 h-4 w-4" /></Link>
                </Button>
              </div>

              <div className="flex flex-wrap gap-4 pt-4 text-xs text-muted-foreground">
                <span className="flex items-center gap-1.5"><CheckCircle2 className="h-3.5 w-3.5 text-emerald-500" />No external services</span>
                <span className="flex items-center gap-1.5"><CheckCircle2 className="h-3.5 w-3.5 text-emerald-500" />Free-tier APIs only</span>
                <span className="flex items-center gap-1.5"><CheckCircle2 className="h-3.5 w-3.5 text-emerald-500" />GitHub Pages ready</span>
                <span className="flex items-center gap-1.5"><CheckCircle2 className="h-3.5 w-3.5 text-emerald-500" />RSS news feeds</span>
              </div>
            </div>
              ...
              <Card className="overflow-hidden">
                ...
                <CardContent className="relative p-6 space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-xs uppercase tracking-wider text-muted-foreground font-semibold">Platform Snapshot</span>
                  ...
                  <Button variant="secondary" size="sm" className="w-full" asChild>
                    <Link href="/dashboard">Open Dashboard <ChevronRight className="ml-1 h-3 w-3" /></Link>
                  </Button>
                </CardContent>
              </Card>
            </div>
      ...
      <section className="container mx-auto max-w-7xl px-4 md:px-6 py-16 md:py-24">
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl md:text-4xl font-bold tracking-tight">Everything a data analyst needs, in one template</h2>
          <p className="text-muted-foreground mt-3 max-w-2xl mx-auto">
            Nine dashboard sections, live market data, RSS news, file management, automation rules,
            and SLO monitoring — all backed by free-tier public APIs.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <FeatureCard icon={Activity} title="Real-time Pipelines" description="Flink-style pipeline monitoring with throughput, lag, p99 latency, and checkpoint health." href="/dashboard" />
          <FeatureCard icon={Database} title="Lakehouse Datasets" description="Apache Iceberg-style table catalog with hot/warm/cold tiers, schema viewer, partition keys." href="/dashboard" />
          <FeatureCard icon={Brain} title="ML Model Registry" description="Production + shadow models with drift monitoring and prediction feed." href="/dashboard" />
        ...
        <div className="container mx-auto max-w-7xl px-4 md:px-6 py-16">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h2 className="text-3xl font-bold tracking-tight mb-4">Five layers, three cross-cutting planes</h2>
              <p className="text-muted-foreground leading-relaxed mb-6">
                Built on the same architectural patterns as production fintech platforms —
              ...
              <Button variant="outline" asChild>
                <Link href="/docs">Read the docs <ArrowRight className="ml-2 h-4 w-4" /></Link>
              </Button>
            </div>
            <div className="space-y-2">
              {[
                ...
                <div key={row.layer} className="flex items-center gap-4 rounded-lg border bg-background p-3 hover:bg-accent/40 transition-colors">
                  <div className="flex h-10 w-10 items-center justify-center rounded-md bg-primary text-primary-foreground text-xs font-bold font-mono">{row.layer}</div>
      ...
      <section className="container mx-auto max-w-4xl px-4 md:px-6 py-16 md:py-24 text-center">
        <h2 className="text-3xl md:text-4xl font-bold tracking-tight mb-4">Ready to explore?</h2>
        <p className="text-muted-foreground mb-8 max-w-xl mx-auto">
          The dashboard runs entirely in your browser. No sign-up, no data sent anywhere.
          Click below to dive into synthetic market data, ML drift, live news, and SLO burn-down.
        </p>
        <div className="flex flex-wrap gap-3 justify-center">
          <Button size="lg" asChild><Link href="/dashboard">Open Dashboard <ArrowRight className="ml-2 h-4 w-4" /></Link></Button>
          <Button size="lg" variant="outline" asChild><Link href="/about">About this project</Link></Button>
        </div>
      </section>
    </div>
  )
